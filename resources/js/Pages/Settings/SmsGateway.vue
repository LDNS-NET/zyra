...
async function openDetails() {
    ...
    try {
        ...
        if (response.ok) {
            ...
            detailsGateway.value = props.gateways[0]
                ? { ...props.gateways[0] }
                : { ...defaultGateway };
        }
    } catch {
...
function setFormFromGateway(gateway) {
    ...
    form.is_active =
        typeof gateway.is_active === 'boolean' ? gateway.is_active : false;
}
...
const save = () => {
    ...
    const loadingToastId = toast.info(
        `Saving SMS gateway settings for ${providerName}...`,
    ...
    const handleSuccess = (page) => {
        ...
        if (editing.value) {
            toast.success(
                `Successfully updated SMS gateway: ${providerName}.`,
                toastOptions,
            );
        } else {
            toast.success(
                `Added new SMS gateway: ${providerName}.`,
                toastOptions,
            );
        }
    };
    const handleError = (errs) => {
        ...
        toast.error(msg || 'Failed to update SMS gateway.', {
            ...toastOptions,
            timeout: 7000,
        });
    };
...
const remove = (gateway) => {
    if (confirm('Delete this SMS gateway?')) {
        const loadingToastId = toast.info('Deleting SMS gateway...', {
            ...toastOptions,
            timeout: 2000,
        });
...
const activate = (gateway) => {
    const loadingToastId = toast.info('Activating SMS gateway...', {
        ...toastOptions,
        timeout: 2000,
    });
        ...
        <transition name="fade">
            <div v-if="form.provider" class="space-y-4">
                ...
                <template v-else>
                    <div>
                        <label
                            class="block font-semibold text-gray-700 dark:bg-gray-800"
                            >API Key</label
                        >
                        <input
                            v-model="form.api_key"
                ...
                <div class="mt-6 flex justify-between">
                    <button
                        class="btn btn-outline btn-info mr-2"
                        type="button"
                        @click="openDetails"
                    >
                        Show Details
                    ...
                    <button
                        class="btn btn-indigo btn-lg shadow transition hover:scale-105"
                        type="button"
                        @click="save"
                    >
                        <svg
                ...
                >
                    ...
                    <div
                        class="space-y-4 rounded-lg bg-gradient-to-br from-blue-50 to-indigo-100 p-6 shadow"
                    >
                        <div class="flex items-center gap-2">
                            ...
                            >
                            <span
                                class="flex items-center gap-1 rounded bg-indigo-100 px-2 py-1 text-sm font-bold text-indigo-700"
                            >
                                <svg
                                    v-if="
                                        detailsGateway.provider === 'talksasa'
                                    "
                                    class="h-4 w-4"
                                    fill="currentColor"
                                    viewBox="0 0 20 20"
                                ...
                                {{
                                    detailsGateway.provider?.toUpperCase() ||
                                    'N/A'
                                }}
                            </span>
                        </div>
                        <div
                            v-if="detailsGateway.label"
                        ...
                        >
                            ...
                            <span
                                ...
                                >{{ detailsGateway.label }}</span
                            >
                        </div>
                        <div
                            v-if="detailsGateway.username"
                            class="flex items-center gap-2"
                        >
                            <span class="font-semibold text-gray-700"
                                >Username:</span
                            ...
                            <span
                                ...
                                >{{ detailsGateway.username }}</span
                            >
                        </div>
                        <div
                            v-if="detailsGateway.api_key"
                            class="flex items-center gap-2"
                        >
                            <span class="font-semibold text-gray-700"
                                >API Key:</span
                            ...
                            <span
                                ...
                                >{{ detailsGateway.api_key }}</span
                            >
                        </div>
                        <div
                            v-if="detailsGateway.sender_id"
                            class="flex items-center gap-2"
                        >
                            <span class="font-semibold text-gray-700"
                                >Sender ID:</span
                            >
                            <span
                                class="rounded bg-yellow-100 px-2 py-1 text-sm text-yellow-700"
                                >{{ detailsGateway.sender_id }}</span
                            >
                        </div>
                        <div
                            v-if="detailsGateway.api_secret"
                            class="flex items-center gap-2"
                        >
                            <span class="font-semibold text-gray-700"
                                >API Secret:</span
                            >
                            <span
                                class="rounded bg-red-100 px-2 py-1 text-sm text-red-700"
                                >{{ detailsGateway.api_secret }}</span
                            >
                        </div>
                        <div class="flex items-center gap-2">
                            <span class="font-semibold text-gray-700"
