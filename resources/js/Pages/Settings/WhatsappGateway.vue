...
const save = () => {
    form.post(route('settings.whatsapp_gateway.update'), {
        preserveScroll: true,
        onSuccess: () => {
...
<template>
    <div class="mx-auto max-w-2xl rounded bg-white p-6 shadow dark:bg-gray-800">
        <h3 class="mb-4 text-xl font-bold">Tenant WhatsApp Gateway</h3>
        <div
        ...
        >
            ...
            <div>
                ...
                {{
                    props.gateways[0].provider === 'twilio'
                        ? 'Twilio (Default)'
                        : props.gateways[0].provider
                }}
            </div>
            ...
            <div v-if="props.gateways[0].label">
                <span class="font-medium">Label:</span>
                {{ props.gateways[0].label }}
            </div>
            <div v-if="props.gateways[0].webhook_url">
                <span class="font-medium">Webhook URL:</span>
                {{ props.gateways[0].webhook_url }}
            </div>
            <div v-if="props.gateways[0].status_callback_url">
                <span class="font-medium">Status Callback:</span>
                {{ props.gateways[0].status_callback_url }}
            </div>
            <div v-if="props.gateways[0].is_active">
                <span class="font-bold text-green-600">Active</span>
            </div>
        </div>
        <form @submit.prevent="save">
            <div class="mb-4">
                <label class="mb-1 block font-medium">WhatsApp Provider</label>
                <select
            ...
            <div v-if="showCredentials">
                <div class="mb-4">
                    <label class="mb-1 block font-medium">API Key / SID</label>
                    <input
                ...
                <div class="mb-4">
                    <label class="mb-1 block font-medium"
                        >Status Callback URL (optional)</label
                    >
                    <input
                        v-model="form.status_callback_url"
                        class="input input-bordered w-full"
                        placeholder="https://..."
                    />
                </div>
