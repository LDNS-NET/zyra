...
function bulkDelete() {
    ...
    if (
        confirm(
            `Are you sure you want to delete ${selectedInvoice.value.length} selected invoices?`,
        )
    ) {
        router.delete(route('invoices.bulk-destroy'), {
            ...
            onError: () => {
                toast.error(
                    'There was an error deleting the selected invoices.',
                );
            },
        });
    }
...
function toggleSelectAll(event) {
    if (event.target.checked) {
        selectedInvoice.value =
            props.invoices && props.invoices.data
                ? [...props.invoices.data]
                : [];
    } else {
        selectedInvoice.value = [];
...
watch(selectedInvoice, (val) => {
    selectAll.value =
        props.invoices &&
        props.invoices.data &&
        val.length === props.invoices.data.length;
});
</script>
    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center justify-between">
                ...
                >
                    <ReceiptText
                        class="mr-2 inline h-6 w-6 text-cyan-700 dark:text-indigo-400"
                    />
                    Invoices
                </h2>
                <PrimaryButton
                    @click="openModal()"
                    v-if="props.can && props.can.create_invoice"
                >
        ...
        <div
            v-if="selectedInvoice.length > 0"
            class="mb-4 flex items-center justify-between rounded border border-yellow-300 bg-yellow-100 px-4 py-2 text-yellow-800"
        >
        ...
        <div class="py-6">
            <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
                ...
                >
                    <div
                        class="border-b border-gray-200 bg-white p-6 dark:border-gray-700 dark:bg-gray-800"
                    >
        ...
        <Modal :show="modalOpen" @close="closeModal">
            ...
            <form @submit.prevent="submit">
                <div class="mb-4">
                    ...
                    <SelectInput
                        ...
                        :options="[
                            ...
                            ...(Array.isArray(props.networkUsers)
                                ? props.networkUsers
                                : []
                            ).map((user) => ({
                                value: user.id,
                ...
                <div class="flex justify-end">
                    ...
                    <DangerButton type="button" class="ml-2" @click="closeModal"
                        >Cancel</DangerButton
                    >
                </div>
            </form>
